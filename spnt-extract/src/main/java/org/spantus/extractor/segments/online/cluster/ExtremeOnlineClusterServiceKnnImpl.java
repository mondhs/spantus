...
public class ExtremeOnlineClusterServiceKnnImpl extends
		ExtremeOnlineClusterServiceSimpleImpl {

	protected Logger log = Logger
			.getLogger(ExtremeOnlineClusterServiceKnnImpl.class);
...
//			NormalizeMidrange nmr = new NormalizeMidrange(0.5, 1);
			// nmr.build(getDataSet());
			// nmr.filter(normalized);
			...
			for (Dataset cluster : clusters) {
				...
				log.debug("[getClassName] cluster[{0}] class{1}:  {2}", i,
						remap.get(i), cluster);
...
//			Instance copiedTmpInstance = tmpInstance.copy();
			// nmr.filter(tmpInstance);

			Classifier knnClassifier = new KNearestNeighbors(3);
			knnClassifier.buildClassifier(testDataset);
			Double predictedClassValue = (Double) knnClassifier
					.classify(tmpInstance);
			log.debug("[getClassName] instance: {0}", tmpInstance);
			log.debug("[getClassName] predictedClassValue: {0}",
					predictedClassValue);
			log.debug("[getClassName]---");
