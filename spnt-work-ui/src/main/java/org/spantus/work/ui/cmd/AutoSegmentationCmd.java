    ...
    public String execute(SpantusWorkInfo ctx) {
        ...
        for (IExtractor extractor : reader.getExtractorRegister()) {
            if (extractor instanceof IClassifier) {
                ...
                ctx.getProject().getSample().getMarkerSetHolder().getMarkerSets().put(markerSet.getMarkerSetType(), markerSet);
        ...
        if (classifiers != null && classifiers.size() > 0) {
            ...
            ISegmentatorService segmentator = SegmentFactory.createSegmentator(ctx.getProject().getFeatureReader().getWorkConfig().getSegmentationServiceType());
