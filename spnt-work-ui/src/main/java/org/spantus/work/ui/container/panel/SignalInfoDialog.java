...
public class SignalInfoDialog extends SpantusAboutDialog {
    ...
    public void setCtx(SpantusWorkInfo ctx) {
        ...
        double sampleRateInHz = ((Float) signalFormat.getParameters().get("sampleRateInHz")).doubleValue();
        ...
        sb.append(createWindowInformation(ctx.getProject(), 
                ctx.getProject().getFeatureReader().getWorkConfig(), sampleRateInHz));
    ...
    private StringBuilder createWindowInformation(SpantusWorkProjectInfo project, WorkUIExtractorConfig config, double sampleRateInHz) {
        StringBuilder sb = new StringBuilder();
        Map<String, Object> info = new LinkedHashMap<String, Object>();
        IExtractorConfig readerConfig = WorkUIExtractorConfigUtil.convert(config, sampleRateInHz);
        ...
        info.put("classifier", WindowOptionPnl.PREFIX_classifier + project.getClassifierType());
        info.put("segmentation", WindowOptionPnl.PREFIX_segmentation + config.getSegmentationServiceType());
        int overlapInSamples = readerConfig.getWindowSize()-readerConfig.getWindowOverlap();
        ...
        sb.append("</div>");
        sb.append("<div>");
        sb.append("<span class=\"label\">").append(getMessage("windowSizeInSamples")).append(":</spant>");
        sb.append("<span class=\"value\">").append("" + readerConfig.getWindowSize()).append("</spant>");
        sb.append("</div>");
        sb.append("<div>");
