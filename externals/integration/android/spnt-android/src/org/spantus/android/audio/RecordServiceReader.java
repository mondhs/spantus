...
public class RecordServiceReader extends RecordServiceUrl {
	...
	public ExtractorReaderCtx createReader(SpantusAudioCtx ctx) {
		...
		ExtractorReaderCtx readerCtx = AndroidExtractorsFactory.createReader(
	 ...
	 * @param ctx
	 */
	public void recordToReader(SpantusAudioCtx ctx,
			WraperExtractorReader wrappedReader) {
		beforeRecord(ctx);
		...
		try {
			...
			while (RecordState.RECORD.equals(ctx.getRecordState())) {
				...
				if (wrappedReader.getSample() > ctx.getMaxLengthInSamples()) {
					LOG.debug("[recordToFile]time out: {0} >{1}",
							wrappedReader.getSample(),
	 ...
	 * @return
	 */
	public URL sendFullSignal(IExtractorInputReader iExtractorInputReader,
			SpantusAudioCtx ctx) {
		LOG.debug("[sendFullSignal] config {0}",
				iExtractorInputReader.getConfig());
	...
	public URL sendSegment(SignalSegment segment, SpantusAudioCtx ctx)
			throws IOException {
	...
	public void readFile(URL inputFile, RecordServiceReader aRecordService,
