...
public class BuferedAudioWriterRunnable implements Runnable {
	...
	public BuferedAudioWriterRunnable(SpantusAudioCtx ctx, AudioInputStream in,
	...
	public void run() {
		try {
			...
			conn.setRequestProperty("Content-Type",
					getContentType(this.ctx.getRecordFormat()));
			...
			LOG.debug("[run]Posted total of  {0}  audio bytes",
					bytes.size());
			LOG.debug("[run]Http response line: {0} - {1}",
