...
//        learnDir = new File(getDirLearn());
        ...
        if(extractor == null){
            ...
            extractorImpl.setClassifier(getExpConfig().getClassifier());
            extractorImpl.setParams(getExpConfig().getExtractorPramMap());
            extractorImpl.setSegmentatorServiceType(getExpConfig().getSegmentatorServiceType());
        	log.error("CorpusEntryExtractorFileImpl created. rulePath: {0}; RulesTurnedOn: {1}; Classifier: {2}",
                    extractorImpl.getRulePath(),
                    extractorImpl.isRulesTurnedOn(),
                    extractorImpl.getClassifier());
