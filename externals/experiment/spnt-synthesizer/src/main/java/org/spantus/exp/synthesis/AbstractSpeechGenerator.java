...
public abstract class AbstractSpeechGenerator {
    ...
    protected abstract void processMarker(MarkerMbrola phoneMarker, float lengthCoef);
...
//		return new DDSAudioInputStream(filtered, ais.getFormat());
        ...
        String command = MessageFormat.format(
                "octave --silent --eval [y,Fs,bits]=wavread(''{0}'');y=awgn(y'''',{1},''measured'');wavwrite(y'''',Fs,''{0}'');", filename, snr);
        Process p;
        try {
            ...
            String stdOutputLine = null;
            StringBuilder stdOutputBuffer = new StringBuilder();
            // read the output from the command
            while ((stdOutputLine = stdInput.readLine()) != null) {
            	stdOutputBuffer.append(stdOutputLine).append("\n");
            }
            if(stdOutputBuffer.length()>0){
            	LOG.debug("[addNoize]Here is the standard output of the command:\n {0}", stdOutputBuffer);
            }
...
//				0f;
     ...
     * @param phone
         * @param lengthCoef 
     */
    protected MarkerMbrola parseAndAppend(Transcribtion transcribtion, String phone, float lengthCoef) {
        ...
        MarkerMbrola phoneMarker = newMarkerSynthesis(transcribtion.getFinished(),
                previousPhoneLength, phoneLabel, splitedIter, lengthCoef);
     ...
     * @return
     */
    private MarkerMbrola newMarkerSynthesis(long start, Long length, String phoneLabel, Iterator<String> splitedIter, float lengthCoef) {
        ...
        processMarker(phoneMarker, lengthCoef);
        return phoneMarker;
    }
