...
import org.spantus.math.dtw.abeel.lang.TypeConversions;
...
class SwapFileMatrix implements CostMatrix {
	...
	public double get(int col, int row) {
		...
		try {
			...
			cellValuesFile.seek(colOffsets[col]
					+ (long) (8 * (row - window.minJforI(col))));
		...
		if (col > currColIndex)
			throw new InternalError(
					"The requested value is in the search window but has not been entered into the matrix: (col="
							+ col + "row=" + row + ").");
		else
			throw new InternalError(
