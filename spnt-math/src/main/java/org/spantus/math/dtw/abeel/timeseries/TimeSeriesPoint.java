...
public class TimeSeriesPoint {
	...
	public TimeSeriesPoint(double value) {
		...
		hashCode += (new Double(value)).hashCode();
    ...
    public TimeSeriesPoint(Collection<? extends Number> values) {
        measurements = new double[values.size()];
        hashCode = 0;
        Iterator<? extends Number> i = values.iterator();
        for (int index = 0; i.hasNext(); index++) {
            Number nextElement = i.next();
            measurements[index] = nextElement.doubleValue();
