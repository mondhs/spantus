...
public class DTW {
    ...
    public double getWarpDistBetween(ITimeSeries tsI, ITimeSeries tsJ,
        ...
        int maxI = tsI.size() - 1;
        int maxJ = tsJ.size() - 1;
        for (Iterator<ColMajorCell> matrixIterator = window.iterator(); matrixIterator.hasNext();) {
            ...
            if (i == 0 && j == 0) {
                ...
                double minGlobalCost = Math.min(costMatrix.get(i - 1, j), Math.min(costMatrix.get(i - 1, j - 1), costMatrix.get(i,
                        j - 1)));
