...
public class WarpPath {
	...
	public ArrayList<Integer> getMatchingIndexesForI(int i) {
		...
		for (; index < tsIindexes.size()
				&& tsIindexes.get(index).equals(new Integer(i)); matchingJs
				.add(tsJindexes.get(index++)))
			;
		return matchingJs;
	}
	...
	public ArrayList<Integer> getMatchingIndexesForJ(int j) {
		...
		for (; index < tsJindexes.size()
				&& tsJindexes.get(index).equals(new Integer(j)); matchingIs
				.add(tsIindexes.get(index++)))
			;
		return matchingIs;
	}
	...
	public ColMajorCell get(int index) {
		if (index > size() || index < 0)
			...
			return new ColMajorCell( tsIindexes.get(index),
					tsJindexes.get(index));
	}
