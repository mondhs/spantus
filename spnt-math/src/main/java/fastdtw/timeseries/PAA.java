...
public class PAA extends TimeSeries {
	...
	public PAA(ITimeSeries ts, int shrunkSize) {
		...
		if (ts instanceof TimeSeries) {
			setLabels(((TimeSeries) ts).getLabels());
		}
		...
		for (int ptToReadFrom = 0; ptToReadFrom < ts.size(); ptToReadFrom = ptToReadTo + 1) {
			ptToReadTo = (int) Math
					.round(reducedPtSize * (double) (size() + 1)) - 1;
			...
			for (int pt = ptToReadFrom; pt <= ptToReadTo; pt++) {
				...
				int dim = 0;
				for (Float val1 : ts.getMeasurementVector(pt)) {
					measurementSums[dim] = val1;
					dim++;
				}
			}
			...
			for (int dim = 0; dim < ts.numOfDimensions(); dim++)
				measurementSums[dim] = measurementSums[dim]
						/ (double) ptsToRead;
	...
	public String toString() {
		return "(" + originalLength + " point time series represented as "
				+ size() + " points)\n" + super.toString();
	}
